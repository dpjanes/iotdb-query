...
const _expect = (stream, token) => {
    ...
    if (current.token !== token) {
        throw new Error(`expected "${token}" not "${current.token}"`)
    }
...
/*
console.log("---")
console.log("CURREN", expression, stream.position)
console.log("NEXT", next)
*/

        if (next.token === ",") {
            if (!expect_comma) {
                throw new Error(`was not expecting to see a comma here`)
            }
            ...
            throw new Error("didn't expect to get here: " + next.token)
        }
    }
